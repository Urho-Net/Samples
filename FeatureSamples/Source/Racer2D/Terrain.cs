...
namespace Racer2D
{
    public class Terrain
    {
        ...
        public Terrain(Scene scene)
        {
            ...
            _surfaceMaterial = Cache.GetMaterial("Materials/UnlitAlpha.xml");
            _surfaceMaterial.SetTexture(0, Cache.GetTexture2D("Scenarios/grasslands/surface.png"));
            _chunkMaterial = Cache.GetMaterial("Materials/Unlit.xml");
            _chunkMaterial.SetTexture(0, Cache.GetTexture2D("Scenarios/grasslands/ground.png"));

            // We create and populate a library with the decorations
            Func<string, Sprite2D> GetSprite = file => Cache.GetSprite2D($"Scenarios/grasslands/Object/{file}");
            ...
            {
                sprite.HotSpot = (new Vector2(0.5f, 0.1f));
            }

            // We generate the chunks and add some boxes randomly
            Sprite2D crateSprite = Cache.GetSprite2D("Scenarios/grasslands/Object/Crate.png");
            for (int i = 0; i < Chunksize * chunksToGenerate; i += Chunksize)
            {
                ...
                {
                    Node crateNode = Racer2D.CreateSpriteNode(crateSprite, 3);
                    crateNode.Position = (new Vector3(i + _rng.Next(8), 20, -5));
                    CollisionBox2D crateCollider = crateNode.CreateComponent<CollisionBox2D>();
                    crateCollider.SetSize(0.76f, 0.76f);
                    crateCollider.Density = (1.0f);
                    crateCollider.Restitution = (0.6f);
                    crateCollider.Friction = (0.4f);
                }
            }
        }
        ...
        void GenerateChunk(int startx)
        {
            ...
            groundComponent.BeginGeometry(0, PrimitiveType.TriangleList);

            var surfaceComponent = node.CreateComponent<CustomGeometry>();
            surfaceComponent.SetMaterial(_surfaceMaterial);
            surfaceComponent.BeginGeometry(0, PrimitiveType.TriangleList);
            ...
            surfaceCollider.Loop = (false);
            surfaceCollider.Friction = (1);
            surfaceCollider.VertexCount = ((uint)surfacePoints.Count + 1);
            ...
            node.CreateComponent<RigidBody2D>().BodyType = (BodyType2D.Static);
        }
        ...
        void CreateDecor(Vector3 position, Vector3 leftVector)
        {
            ...
            if (chance < 1.2f)
            {
                ...
                if (_rng.Next(6) < 4)
                    node = Racer2D.CreateSpriteNode(_decorationLib.GetRandomFromGroup(0), 3, false);
            }
            else if (chance < 1.3f)
                node = Racer2D.CreateSpriteNode(_decorationLib.GetRandomFromGroup(1), 4, false);
            else if (chance < 1.7f)
            {
                // We only spawn trees if the surface isn't too steep
                if (Math.Abs(leftVector.Y) < 0.1f)
                {
                    node = Racer2D.CreateSpriteNode(_decorationLib.GetRandomFromGroup(2), 4, false);
                    node.SetScale2D((node.Scale2D.X * 1 + _rng.Next(30) / 100f), (node.Scale2D.Y * 1 + _rng.Next(30) / 100f));
                    isTree = true;
                }
            }
