...
public class Clouds
{
    ...
    public Clouds(float startX, int minY, int deviation, int amount, int range)
    {
        ...
        Sprite2D[] cloudSprites = {
            Cache.GetSprite2D("Scenarios/cloud1.png"),
            Cache.GetSprite2D("Scenarios/cloud2.png"),
            Cache.GetSprite2D("Scenarios/cloud3.png")};
    ...
    private void RecycleCloud(float currentX, Node cloud)
    {
        ...
        cloud.Position = new Vector3(currentX+_range, rng.Next(_deviation) + _minY, 15);
    }
