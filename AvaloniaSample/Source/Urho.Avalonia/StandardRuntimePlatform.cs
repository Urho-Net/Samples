...
namespace Urho.Avalonia
{
    internal partial class StandardRuntimePlatform : IRuntimePlatform
    {
        ...
        {
            return new System.Threading.Timer(_ => tick(), null, interval, interval);
        }
        ...
        class UnmanagedBlob : IUnmanagedBlob
        {
            ...
            public void Dispose()
            {
                ...
                if (Thread.CurrentThread.ManagedThreadId == GCThread?.ManagedThreadId)
                {
                    lock (_lock)
                    {
                        if (!IsDisposed)
                        {
                            global::System.Console.Error.WriteLine("Native blob disposal from finalizer thread\nBacktrace: "
            ...
            {
#if DEBUG
                System.Console.Error.WriteLine("Undisposed native blob created by " + _backtrace);
#endif
                DoDispose();
            }
