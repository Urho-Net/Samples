...
");
        ...
        {
            var urhoAssembly = Urho.Application.Current.GetType()?.Assembly;

            var name = urhoAssembly.GetManifestResourceNames().First(x => x.Contains("teapot.bin"));
            using (var sr = new BinaryReader(urhoAssembly.GetManifestResourceStream(name)))
            {
